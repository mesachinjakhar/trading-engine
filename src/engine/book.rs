...
impl OrderBook {
    ...
    pub fn cancel_order(&mut self, order_id: u64) -> Result<(), &'static str> {
        ...
        if let Some(order) = is_buy {
            if order.state != OrderState::Created || order.state != OrderState::Accepted {
                return Err("No can not cancel at this stage");
            }
            order.cancel().unwrap()
        }
        if let Some(order) = is_sell {
    ...
    fn best_buy_id(&self) -> Option<u64> {
        self.buys
            .iter()
            .max_by(|(_, a), (_, b)| a.price.cmp(&b.price).then_with(|| b.seq.cmp(&a.seq)))
            .map(|(id, _)| *id)
    }
    ...
    fn best_sell_id(&self) -> Option<u64> {
        self.sells
            .iter()
            .min_by(|(_, a), (_, b)| a.price.cmp(&b.price).then_with(|| a.seq.cmp(&b.seq)))
            .map(|(id, _)| *id)
    }
}
